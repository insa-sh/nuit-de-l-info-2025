...
## Première faille: achats non vérifiées côté serveur
...
Comme de par hasard, le chemin d'api `/scoreboard` renvoie des informations sous ce format:
...
Pour résoudre le problème, la solution la plus simple est d'utiliser un UUIDv4, qui ne permet pas ce type d'attaque.
