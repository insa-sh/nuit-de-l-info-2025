...
import { chick, colors, farm, fence, floorTile, grassTile, hayTile, hole, leftSign, letters, mine, pedestal, pedestalRoom, playerSprite, farmPnj, rightSign, rock, rockTile, wall, minePnj, SpawnPnj } from "./const";
...
export default function Page() {
  ...
  let remainingHit = 0;
  let title: string[] | undefined = ["Participez aux quetes", "et récuperez votre pseudo"]
  const username: number[] = []

  function openPedestalRoom() {
    const game = createGame({
      ...
      templates: {
        ...
        g: {
          ...
          dialog : "%Bienvenue dans la salle du pseudo|Pour sauvegarder ton score, tu dois aller récuperer les lettres dans la mine|Il te faudra aussi acheter une pioche grâce à l'argent gagné à Pole emploi%"
        }
      },
      map: pedestalRoom,
      background: 1,
    ...
    for (let index = 0; index < username.length; index++) {
      game.updateCellAt(1 + index, 1, { sprite: mergeSprites(pedestal, letters[username[index]]) })
    }
    if (username.length >= 8) {
      const user = username.map((val) => {
        if (val > 9) {
          ...
          h: {
            ...
            async onCollide(target) {
              if (hasPickaxe) {
                ...
                if (username.length >= 8) {
                  game.openDialog("Votre pseudo est déja complet|Revenez en salle des pseudo pour le valider")
                  return
                }
                let char = ""
                if (charNumber > 9) {
        ...
        templates: {
          ...
          c: {
            ...
            onCollide(target) {
              ...
              if (!nextCell.solid && (nextCell.position[0] != 9 || nextCell.position[1] != 5)) {
                game.setCellAt(tx + dx, ty + dy, target.symbol)
                if (!gameFinish && nextCell.position[0] == 3 && nextCell.position[1] == 2) {
                  if (dx < 0) {
                    chickCount++
                    if (chickCount == 3) {
                      coinCount++
                      gameFinish = false
                      game.openDialog("Bravo vous avez bien mérité cette pièce !")
